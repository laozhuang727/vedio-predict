...
max_len[i] = {'applist': 100, 'tag': 100, 'outertag': 100}
    ...
    try:
        ...
        for i in ['applist', 'tag', 'outertag']:
            ...
            varlen_list['%s_key' % i] = pad_sequences(np.array(
                data['%s_key' % i]), maxlen=max_len[i], padding='post', dtype=str, value=0)
            varlen_list['%s_weight' % i] = pad_sequences(np.array(
                data['%s_weight' % i]), maxlen=max_len[i], padding='post', dtype=str, value=0).reshape(len(data),
                                                                                                       max_len[i], 1)
            #
            # varlen_list['%s_key' % i] = data['%s_key' % i]
            # varlen_list['%s_weight' % i] = np.array(data['%s_weight' % i]).reshape(max_len[i], 1)
...
if isVarlen:
    varlen_feature_columns = [
        VarLenSparseFeat('%s_key' % i, maxlen=max_len[i], vocabulary_size=100,
                         embedding_dim=4, combiner='mean', use_hash=True, dtype="string")
        for i in ['applist', 'tag', 'outertag']]
    ...
    #                                            combiner='mean', weight_name='%s_weight' % col_name)
    #                           for col_name in ['applist', 'tag', 'outertag']]
...
model = DeepFM(linear_feature_columns, dnn_feature_columns,
               fm_group=fixlen_feature_columns, dnn_hidden_units=(256, 256, 256), l2_reg_linear=0.001,
...
sub.to_csv(path_sub + 'sub_hash.csv', index=False)
